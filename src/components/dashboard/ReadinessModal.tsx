...
const Toggle = ({ active, onChange, label, sublabel }: { active: boolean; onChange: (v: boolean) => void; label: string; sublabel: string }) => (
  <button
    ...
    className={`group flex items-center justify-between p-5 rounded-2xl border-2 transition-all text-left ${
      active
        ? "bg-slate-900 dark:bg-zinc-800 border-slate-900 dark:border-zinc-600 text-white shadow-xl shadow-slate-200 dark:shadow-none"
        : "bg-white dark:bg-[#1c1c1c] border-slate-100 dark:border-white/6 text-slate-600 dark:text-zinc-300 hover:border-slate-200 dark:hover:border-white/20"
    }`}
  >
    ...
    <div className={`w-11 h-6 rounded-full relative transition-all shrink-0 ml-4 ${active ? "bg-red-500" : "bg-slate-200 dark:bg-white/10"}`}>
      <motion.div
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            <div
              className="bg-white dark:bg-[#141414] rounded-4xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl dark:shadow-black/60 border dark:border-white/6"
            ...
            >
              ...
              <div className="p-8 pb-0 flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-xl font-black text-slate-900 dark:text-white tracking-tight">Verify Readiness</h2>
                    <p className="text-xs font-bold text-slate-400 dark:text-zinc-500">Pre-dispatch confirmation check</p>
                  </div>
                </div>
                <button
                  onClick={onClose}
                  className="w-9 h-9 rounded-xl bg-slate-100 dark:bg-white/5 flex items-center justify-center text-slate-400 dark:text-zinc-500 hover:bg-slate-200 dark:hover:bg-white/10 shrink-0"
                >
                  <X size={18} />
                </button>
              </div>
              ...
              <form onSubmit={handleSubmit} className="p-8 space-y-6">
                ...
                {stops.length > 1 && (
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 dark:text-zinc-500 uppercase tracking-widest px-1">Checking Stop</label>
                    <select
                      value={selectedStopId}
                      onChange={(e) => setSelectedStopId(e.target.value)}
                      className="w-full px-4 py-3.5 bg-slate-100 dark:bg-[#1c1c1c] border border-slate-300 dark:border-white/6 rounded-2xl text-sm font-bold text-slate-900 dark:text-white focus:ring-2 focus:ring-emerald-500 outline-none"
                    >
                      {stops.map((js) => (
                ...
                {/* Target Date */}
                <div className="flex items-center gap-3 p-4 bg-slate-50 dark:bg-[#1c1c1c] rounded-2xl border border-slate-100 dark:border-white/6">
                  <CalendarDays size={18} className="text-slate-400 dark:text-zinc-500 shrink-0" />
                  <div>
                    ...
                    <input
                      ...
                      className="bg-transparent font-black text-slate-900 dark:text-white text-sm focus:outline-none dark:scheme-dark"
                    />
                  </div>
                ...
                <textarea
                  ...
                  className="w-full bg-slate-100 dark:bg-[#1c1c1c] border border-slate-300 dark:border-white/6 rounded-2xl p-5 text-sm font-bold text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-zinc-600 outline-none focus:ring-2 focus:ring-emerald-500 transition-all resize-none"
                  rows={2}
                />
                ...
                <button
                  ...
                  className={`w-full py-4 rounded-2xl font-black text-base shadow-xl transition-all flex items-center justify-center gap-2 active:scale-[0.98] ${
                    success
                      ...
                      : "bg-slate-900 dark:bg-zinc-800 text-white shadow-slate-200 dark:shadow-none hover:bg-slate-800 dark:hover:bg-zinc-700"
                  }`}
                >
                  {success ? (

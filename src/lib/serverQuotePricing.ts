...
export type AuthoritativeQuote = { total:number; distance:number; durationMinutes:number|null; breakdown:QuoteBreakdown; serviceType:string; vehicleType:string|null };
...
export async function computeAuthoritativeQuote(input:ComputeInput):Promise<QuoteComputationResult>{
  ...
  if(vehicleCount&&vehicleCount>0){
    ...
    if(widgetSettings?.showVehicles){
      ...
      if(vehicleOptions.length>0){
        ...
        if(!requested)return{ok:false,status:400,error:"Please select a vehicle type."};
  ...
  return{ok:true,quote:{total,distance,durationMinutes,serviceType:resolvedService,vehicleType:resolvedVehicleType,breakdown:{total,lineItems,distanceMiles:detailed.distanceMiles,freeMiles:detailed.freeMiles,billableMiles:detailed.billableMiles,minimumApplied:detailed.minimumApplied}}};
}

...
import { CheckCircle, Calendar, MapPin, ArrowRight, Truck, Clock } from "lucide-react";
...
export default async function PaymentSuccessPage({
  ...
  // Access requires a valid signed capability token. A raw quoteId in the URL,
  // a forged/tampered token, an expired token, or the wrong purpose all fail
  // the same non-revealing way (notFound) — no quote existence is disclosed.
  if (!token) {
    notFound();
  }
  ...
  // Tenant binding: fetch by quoteId AND companyId together, so a token for
  // Company A can never surface Company B's quote. Only fields rendered below
  // are selected — no broad company include, no sensitive Company fields.
  ...
  // The database/webhook payment status is authoritative — the Stripe redirect
  // alone never implies success.
  const isPaid = quote.paymentStatus === "PAID";
  const isFailed = quote.paymentStatus === "FAILED";

  // Not-yet-paid (or failed): render a neutral, minimal state. No success claim,
  // no confetti, and no extra sensitive details beyond the amount.
  if (!isPaid) {
    return (
            ...
            <div className="relative z-10 flex flex-col items-center">
              ...
              <p className="text-white/90 font-medium mt-2">
                {isFailed
                  ...
                  : "This can take a moment to finalize."}
              </p>
            </div>
          </div>

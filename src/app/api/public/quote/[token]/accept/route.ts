...
export async function POST(
  ...
  if (["CONFIRMED", "WON", "PAID"].includes(quote.status)) {
    return NextResponse.json({ success: true, status: quote.status }, { headers: PRIVATE_HEADERS });
  }
  ...
  if (updated.count === 1) {
    fireWebhooks(document.companyId, "quote.status_changed", {
      quote: { id: quote.id, status: "CONFIRMED", previousStatus: quote.status },
    });
  }

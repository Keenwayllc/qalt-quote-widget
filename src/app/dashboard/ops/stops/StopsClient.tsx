    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-12 pr-4 py-3.5 bg-white border border-slate-200 rounded-2xl shadow-sm focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500 transition-all font-medium text-slate-700"
          />
        </div>
        <button
      ...
      {stops.length === 0 ? (
        <div className="bg-white rounded-[32px] border border-slate-200/60 shadow-xl p-12 text-center">
          ...
          <h2 className="text-2xl font-black text-slate-900 mb-2">No Saved Stops Yet</h2>
          ...
          <AnimatePresence mode="popLayout">
            {filteredStops.map((stop) => (
              <motion.div
                ...
                className="bg-white rounded-[2rem] border border-slate-200/60 shadow-sm hover:shadow-xl hover:border-red-100 transition-all group overflow-hidden flex flex-col h-full cursor-pointer relative isolate"
                onClick={() => router.push(`/dashboard/ops/stops/${stop.id}`)}
              >
                <div className="p-6 sm:p-8 flex-1">
                  <div className="flex items-start justify-between mb-6">
                    <div className="h-14 w-14 rounded-2xl bg-slate-50 text-slate-400 group-hover:bg-red-50 group-hover:text-red-500 transition-colors flex items-center justify-center shrink-0">
                      <Building2 size={28} />
                    </div>
                  ...
                  <div className="space-y-1 mb-6">
                    <h3 className="font-black text-slate-900 text-xl tracking-tight leading-tight group-hover:text-red-600 transition-colors truncate">
                      {stop.companyName}
                    </h3>
                  ...
                  <div className="space-y-3 pt-6 border-t border-slate-50">
                    <div className="flex items-center justify-between text-xs font-bold">
                       ...
                       <span className="text-slate-800">{stop.hours || "--"}</span>
                    </div>
                    <div className="flex items-center justify-between text-xs font-bold">
                       <span className="text-slate-400 flex items-center gap-1.5">
                        <Shield size={14} /> Gate Code
                       </span>
                       <span className="text-slate-800">{stop.gateCode || "None"}</span>
                    </div>
                  </div>
                </div>

                <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex items-center justify-between group-hover:bg-red-50/30 transition-colors mt-auto">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className="relative bg-white rounded-[2.5rem] w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl p-6 sm:p-10 scrollbar-hide"
          >
            <div className="flex items-center justify-between mb-8">
              <div>
                <h2 className="text-2xl font-black text-slate-900 tracking-tight">New Stop Note</h2>
                <p className="text-slate-400 text-sm font-bold">Save delivery details for this location</p>
              </div>
              <button 
                onClick={() => setIsModalOpen(false)}

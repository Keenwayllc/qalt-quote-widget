...
export default function ReadinessClient({ stops }: { stops: any[] }) {
  ...
  const Toggle = ({ active, onChange, label, sublabel }: any) => (
    <button
      ...
      className={`group flex items-center justify-between p-6 rounded-4xl border-2 transition-all text-left ${
        active
          ? "bg-slate-900 border-slate-900 text-white shadow-xl shadow-slate-200 dark:shadow-none dark:bg-zinc-800 dark:border-zinc-600"
          : "bg-white border-slate-100 text-slate-600 hover:border-slate-200 dark:bg-[#1c1c1c] dark:border-white/8 dark:text-zinc-300 dark:hover:border-white/20"
      }`}
    >
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
      ...
      <div className="lg:col-span-2 space-y-6">
        <div className="bg-white dark:bg-[#141414] rounded-[2.5rem] border border-slate-200/60 dark:border-white/6 shadow-sm dark:shadow-none p-8">
          <p className="text-[10px] font-black text-slate-400 dark:text-zinc-500 uppercase tracking-widest mb-4">Select Location</p>
          <div className="relative">
            <Building2 className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 w-5 h-5 pointer-events-none" />
            <select
              value={selectedStopId}
              onChange={(e) => setSelectedStopId(e.target.value)}
              className="w-full pl-12 pr-4 py-4 bg-slate-50 dark:bg-[#1c1c1c] border border-slate-100 dark:border-white/6 rounded-2xl font-black text-slate-900 dark:text-white focus:ring-2 focus:ring-red-500 outline-none appearance-none cursor-pointer"
            >
          ...
          {selectedStop && (
            <motion.div 
              ...
              className="mt-8 pt-8 border-t border-slate-50 dark:border-white/6 space-y-4"
            >
               <div className="flex items-start gap-3">
                  <MapPin size={16} className="text-red-500 shrink-0 mt-1" />
                  <p className="text-sm font-bold text-slate-600 dark:text-zinc-300 leading-snug">{selectedStop.address}</p>
               </div>
               <div className="flex items-center gap-3">
                  <User size={16} className="text-slate-300 shrink-0" />
                  <p className="text-sm font-bold text-slate-600 dark:text-zinc-300">{selectedStop.contactName || "No contact"} — {selectedStop.contactPhone || "No phone"}</p>
               </div>
        ...
        {/* Info Card */}
        <div className="bg-zinc-900 dark:bg-[#1c1c1c] dark:border dark:border-white/6 rounded-[2.5rem] p-8 text-white relative overflow-hidden isolate shadow-2xl shadow-slate-200/60 dark:shadow-none">
      ...
      <div className="lg:col-span-3">
        <form onSubmit={handleSubmit} className="bg-white dark:bg-[#141414] rounded-[2.5rem] border border-slate-200/60 dark:border-white/6 shadow-sm dark:shadow-none p-8 sm:p-10 space-y-8">
           <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                 <div className="w-12 h-12 rounded-2xl bg-slate-50 dark:bg-white/5 flex items-center justify-center text-slate-400 dark:text-zinc-500">
                    <CalendarDays size={24} />
                 </div>
                 <div>
                    <p className="text-[10px] font-black text-slate-400 dark:text-zinc-500 uppercase tracking-widest">Target Date</p>
                    <input
                      type="date"
                      value={checkData.scheduledDate}
                      onChange={e => setCheckData({...checkData, scheduledDate: e.target.value})}
                      className="bg-transparent font-black text-slate-900 dark:text-white focus:outline-none dark:scheme-dark"
                    />
                 </div>
              </div>
           ...
           <div className="space-y-3">
              <label className="text-[10px] font-black text-slate-400 dark:text-zinc-500 uppercase tracking-widest px-2">Readiness Notes</label>
              <textarea
                value={checkData.notes}
                onChange={e => setCheckData({...checkData, notes: e.target.value})}
                placeholder="Specific context for this check..."
                className="w-full bg-slate-100 dark:bg-[#1c1c1c] border border-slate-300 dark:border-white/6 rounded-4xl p-6 text-sm font-bold text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-zinc-600 outline-none focus:ring-2 focus:ring-red-500 transition-all resize-none"
                rows={3}
              />
           </div>
           ...
           <button
            ...
            className={`w-full py-5 rounded-4xl font-black text-lg shadow-xl transition-all flex items-center justify-center gap-3 active:scale-[0.98] ${
                !selectedStopId || isSubmitting
                ? "bg-slate-100 dark:bg-white/5 text-slate-300 dark:text-zinc-600 shadow-none cursor-not-allowed"
                : success

  ...
  if (planError) {
    return (
      <div className="p-6 sm:p-10 max-w-2xl mx-auto">
        <div className="bg-white rounded-[32px] border border-slate-200/60 shadow-xl p-10 text-center">
          ...
          <a
            ...
            className="inline-flex items-center gap-2 bg-red-600 hover:bg-red-700 active:scale-95 text-white font-black px-8 py-3 rounded-2xl transition-all"
          >
            Upgrade Plan
          </a>
        </div>
  ...
  return (
    <div className="p-6 sm:p-10 max-w-4xl mx-auto space-y-8">
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex items-center gap-4 mb-2">
          <div className="w-12 h-12 bg-red-50 border border-red-100 rounded-2xl flex items-center justify-center">
            <Webhook size={22} className="text-red-600" />
          </div>
          <div>
            <h1 className="text-3xl font-black text-slate-900 tracking-tight">Webhooks</h1>
            <p className="text-slate-500 font-medium text-sm">Push quote events to Zapier, Onfleet, Shipday, or your own server.</p>
          </div>
        </div>
      </motion.div>
      ...
      <motion.div
        ...
        className="bg-white rounded-[32px] border border-slate-200/60 shadow-xl shadow-slate-200/40 p-8"
      >
        <h2 className="text-lg font-black text-slate-900 mb-6 tracking-tight">Add Endpoint</h2>
        <form onSubmit={handleAdd} className="space-y-5">
          <div>
            ...
            <input
              ...
              className="w-full border border-slate-200 rounded-2xl px-4 py-3 text-sm font-medium text-slate-800 placeholder:text-slate-300 focus:outline-none focus:ring-2 focus:ring-red-500/30 focus:border-red-300 transition-all"
            />
          </div>
          <div>
            ...
            <div className="space-y-2">
              {ALL_EVENTS.map((ev) => (
                ...
                >
                  ...
                  />
                  <div>
                    <p className="text-sm font-black text-slate-800 font-mono">{ev.label}</p>
          ...
          {addError && <p className="text-sm text-red-600 font-bold">{addError}</p>}
          <button
            type="submit"
            disabled={adding}
            className="flex items-center gap-2 bg-red-600 hover:bg-red-700 active:scale-95 disabled:opacity-60 text-white font-black px-6 py-3 rounded-2xl transition-all text-sm"
          >
            ...
            {webhooks.map((hook) => {
              ...
              return (
                <motion.div
                  ...
                  className={`bg-white rounded-[28px] border shadow-lg p-6 transition-all ${
                    hook.enabled ? "border-slate-200/60" : "border-slate-100 opacity-60"
                  }`}
                >
                  {/* Top row */}
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        <a
                          ...
                          className="text-sm font-black text-slate-900 truncate hover:text-red-600 transition-colors flex items-center gap-1 group"
                        >
                          {hook.url}
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {hook.events.map((ev) => (
                          <span key={ev} className="px-2 py-0.5 bg-slate-100 text-slate-600 text-[10px] font-black rounded-full font-mono">
                            {ev}
                          </span>
                        ))}
                      </div>
                  ...
                  {/* Secret row */}
                  <div className="flex items-center gap-2 bg-slate-50 border border-slate-100 rounded-xl px-4 py-2 mb-4">
                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest shrink-0">Secret</span>
                    <span className="flex-1 text-xs font-mono text-slate-600 truncate select-all">
                    ...
                    <button
                      ...
                      className="text-[10px] font-black text-slate-400 hover:text-slate-700 shrink-0 transition-colors"
                    >
                  ...
                  <div className="flex items-center gap-3">
                    <button
                      ...
                      disabled={isTesting}
                      className="flex items-center gap-1.5 text-xs font-black text-slate-600 hover:text-red-600 border border-slate-200 hover:border-red-200 hover:bg-red-50 px-4 py-2 rounded-xl transition-all active:scale-95 disabled:opacity-50"
                    >
                      {isTesting
                    ...
                    {result && (
                      <span className={`text-xs font-bold ${result.ok ? "text-emerald-600" : "text-red-500"}`}>
      ...
      <motion.div
        ...
        className="bg-white rounded-[32px] border border-slate-200/60 shadow-xl shadow-slate-200/40 overflow-hidden"
      >
        <button
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <span className="font-black text-slate-900 text-sm">Payload Schema & Signature Verification</span>
          </div>
          ...
          {docsOpen && (
            ...
            >
              <div className="px-8 pb-8 space-y-6 border-t border-slate-100">
                <div className="pt-6">
                  <h3 className="text-xs font-black text-slate-500 uppercase tracking-widest mb-3">Example Payload</h3>
                  <div className="relative">
                    <pre className="bg-slate-950 text-emerald-400 text-xs rounded-2xl p-5 overflow-x-auto leading-relaxed font-mono">
                      {EXAMPLE_PAYLOAD}
                    </pre>
                ...
                <div>
                  <h3 className="text-xs font-black text-slate-500 uppercase tracking-widest mb-3">Signature Verification (Node.js)</h3>
                  <pre className="bg-slate-950 text-sky-300 text-xs rounded-2xl p-5 overflow-x-auto leading-relaxed font-mono">{`const crypto = require("crypto");
  ...
  return new Response("Unauthorized", { status: 401 });
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="bg-slate-50 rounded-2xl p-4">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Headers Sent</p>
                    <div className="space-y-1 font-mono text-xs text-slate-700">
                      <p><span className="text-slate-400">Content-Type:</span> application/json</p>
                      <p><span className="text-slate-400">X-Qalt-Event:</span> quote.created</p>
                      <p><span className="text-slate-400">X-Qalt-Signature:</span> sha256=…</p>
                    </div>
                  </div>
                  <div className="bg-slate-50 rounded-2xl p-4">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Quick Test</p>

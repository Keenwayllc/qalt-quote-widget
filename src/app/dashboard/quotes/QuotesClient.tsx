...
function CopyButton({ text }: { text: string }) {
  ...
  return (
    <button onClick={copy} className="ml-1 text-slate-300 dark:text-zinc-600 hover:text-red-500 transition-colors">
        ...
        >
          ...
          <div className="sticky top-0 bg-white/95 dark:bg-[#141414]/95 backdrop-blur-md border-b border-slate-100 dark:border-white/6 px-6 py-4 flex items-center justify-between z-10">
            <div>
              ...
              <p className="text-slate-400 dark:text-zinc-500 text-xs font-medium">ID: {quote.id.slice(0, 12)}…</p>
            </div>
            <button
              onClick={onClose}
              className="w-9 h-9 flex items-center justify-center rounded-xl bg-slate-100 dark:bg-white/5 hover:bg-slate-200 dark:hover:bg-white/10 text-slate-500 dark:text-zinc-400 transition-all active:scale-90"
            >
              <X size={18} />
            </button>
          </div>
          ...
          <div className="px-6 py-6 space-y-5">
            ...
            <div className="bg-slate-50 dark:bg-[#1c1c1c] rounded-2xl p-5 border border-transparent dark:border-white/6">
              ...
              <div className="flex items-center gap-3 mb-4">
                <div className="h-11 w-11 rounded-xl bg-slate-900 dark:bg-zinc-700 flex items-center justify-center text-white font-black text-sm shrink-0">
                  {quote.customerName.charAt(0).toUpperCase()}
                </div>
                <div>
                  <p className="font-black text-slate-900 dark:text-white">{quote.customerName}</p>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2 text-sm text-slate-400 dark:text-zinc-500 font-medium">
                  <Clock size={14} className="text-slate-300 dark:text-zinc-600 shrink-0" />
            ...
            {(quote.packageSize || quote.packageWeight || extras.length > 0) && (
              <div className="bg-slate-50 dark:bg-[#1c1c1c] rounded-2xl p-5 border border-transparent dark:border-white/6">
                ...
                <div className="space-y-2">
                  {quote.packageSize && (
                    <div className="flex items-center gap-2 text-sm text-slate-600 dark:text-zinc-300 font-medium">
                      <Package size={14} className="text-slate-300 dark:text-zinc-600" />
                      <span>Size: <span className="font-bold text-slate-800 dark:text-zinc-100">{quote.packageSize}</span></span>
                    </div>
                  )}
                  {quote.packageWeight && (
                    <div className="flex items-center gap-2 text-sm text-slate-600 dark:text-zinc-300 font-medium">
                      <Truck size={14} className="text-slate-300 dark:text-zinc-600" />
                      <span>Weight: <span className="font-bold text-slate-800 dark:text-zinc-100">{quote.packageWeight}</span></span>
                    </div>
                  )}
                  {extras.length > 0 && (
                    <div className="mt-2">
                      <p className="text-xs text-slate-400 dark:text-zinc-500 font-bold mb-1.5">Extras</p>
                      <div className="flex flex-wrap gap-1.5">
                        {extras.map((e) => (
                          <span key={e} className="px-2.5 py-1 bg-red-50 dark:bg-red-500/10 text-red-700 dark:text-red-400 border border-red-100 dark:border-red-500/20 rounded-lg text-xs font-bold">{e}</span>
                        ))}
                      </div>
                    </div>
                  )}
            ...
            {quote.awbNumber && (
              <div className="bg-slate-50 dark:bg-[#1c1c1c] rounded-2xl p-5 border border-transparent dark:border-white/6">
                ...
                <div className="flex items-center gap-2 text-sm font-black text-slate-800 dark:text-zinc-200">
            ...
            <div className="bg-slate-50 dark:bg-[#1c1c1c] rounded-2xl p-5 border border-transparent dark:border-white/6">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-sm text-slate-600 dark:text-zinc-300 font-medium">
                  <CreditCard size={14} className="text-slate-300 dark:text-zinc-600" />
                  {quote.paymentStatus === "PAID" ? (
                    ...
                    <span className="text-slate-400 dark:text-zinc-500 font-medium">No payment collected</span>
                  )}
                </div>
                <span className="text-2xl font-black text-slate-900 dark:text-white">${quote.estimatedPrice.toFixed(2)}</span>
              </div>
            </div>
            ...
            <div className="bg-slate-50 dark:bg-[#1c1c1c] rounded-2xl p-5 mt-5 border border-transparent dark:border-white/6">
              <div className="flex items-center gap-1.5 mb-3">
                ...
                <div className="relative group/tip">
                  <svg xmlns="http://www.w3.org/2000/svg" className="w-3.5 h-3.5 text-slate-300 dark:text-zinc-600 cursor-help" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><circle cx="12" cy="12" r="10"/><path d="M12 16v-4M12 8h.01"/></svg>
              ...
              <div className="grid grid-cols-3 gap-2 mb-4">
                <div className="relative group/confirmed">
                  <button
                    ...
                    className={`w-full py-2 rounded-xl text-xs font-bold transition-all ${quote.status === "CONFIRMED" ? "bg-red-600 text-white" : "bg-white dark:bg-white/5 border border-slate-200 dark:border-white/6 text-slate-600 dark:text-zinc-300 hover:bg-slate-100 dark:hover:bg-white/10"}`}
                  >Confirmed</button>
                ...
                <div className="relative group/won">
                  <button
                    ...
                    className={`w-full py-2 rounded-xl text-xs font-bold transition-all ${quote.status === "WON" ? "bg-emerald-600 text-white" : "bg-white dark:bg-white/5 border border-slate-200 dark:border-white/6 text-slate-600 dark:text-zinc-300 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 hover:text-emerald-700 dark:hover:text-emerald-400"}`}
                  >Won</button>
                ...
                <div className="relative group/lost">
                  <button
                    ...
                    className={`w-full py-2 rounded-xl text-xs font-bold transition-all ${quote.status === "LOST" ? "bg-rose-600 text-white" : "bg-white dark:bg-white/5 border border-slate-200 dark:border-white/6 text-slate-600 dark:text-zinc-300 hover:bg-rose-50 dark:hover:bg-rose-500/10 hover:text-rose-700 dark:hover:text-rose-400"}`}
                  >Lost</button>
              ...
              <div className="flex items-center gap-1.5 mb-2">
                <p className="text-[10px] font-black text-slate-400 dark:text-zinc-500 uppercase tracking-widest">Private Notes</p>
              ...
              <textarea
                ...
                className="w-full h-24 p-3 rounded-xl border border-slate-200 dark:border-white/6 text-sm focus:outline-none focus:ring-2 focus:ring-red-500 bg-white dark:bg-[#0a0a0a] dark:text-white dark:placeholder:text-zinc-600 resize-none"
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <a
                ...
                className="flex items-center justify-center gap-2 py-3 bg-slate-900 dark:bg-zinc-800 text-white rounded-xl font-bold text-sm hover:bg-slate-700 dark:hover:bg-zinc-700 active:scale-95 transition-all"
              >
                <Mail size={15} />
                Email Customer
              </a>
...
export default function QuotesClient({ quotes: initialQuotes, insideDeliveryLabel, addon3Label }: { quotes: Quote[]; insideDeliveryLabel: string; addon3Label: string }) {
  ...
  return (
    <>
      {quotes.length === 0 ? (
        <div className="bg-white dark:bg-[#141414] rounded-4xl border border-slate-200/60 dark:border-white/6 shadow-xl shadow-slate-200/40 dark:shadow-none overflow-hidden text-center relative isolate mb-12">
          ...
          <div className="px-6 py-24 flex flex-col items-center">
            <div className="p-6 bg-linear-to-br from-red-50 to-slate-100 dark:from-red-500/10 dark:to-white/5 rounded-3xl border border-red-100/50 dark:border-red-500/20 shadow-inner text-red-500 mb-8 relative">
              <div className="absolute inset-0 bg-white/20 dark:bg-white/5 rounded-3xl backdrop-blur-md" />
              <div className="relative">
                <Settings size={48} strokeWidth={1.5} className="text-red-600 dark:text-red-400" />
              </div>
            </div>
            <h2 className="text-3xl font-black text-slate-900 dark:text-white mb-4 tracking-tight">You&apos;re almost there!</h2>
            <p className="text-slate-500 dark:text-zinc-400 font-medium text-lg mb-10 max-w-md mx-auto leading-relaxed">
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-center">
              <Link href="/dashboard/widget" className="px-8 py-3.5 bg-slate-900 dark:bg-zinc-800 text-white rounded-xl font-bold hover:bg-slate-800 dark:hover:bg-zinc-700 active:scale-95 transition-all shadow-lg shadow-slate-200 dark:shadow-none flex items-center gap-2">
                <Settings size={18} /> Customize Widget
              </Link>
              <Link href="/dashboard/embed" className="px-8 py-3.5 bg-white dark:bg-white/5 border border-slate-200 dark:border-white/6 text-slate-700 dark:text-zinc-300 rounded-xl font-bold hover:bg-slate-50 dark:hover:bg-white/10 active:scale-95 transition-all flex items-center gap-2">

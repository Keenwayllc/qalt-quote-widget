...
function QuoteCard({
  ...
  return (
    <div ref={setNodeRef} style={style} className="group">
      <div
        className={`bg-white rounded-2xl border border-slate-200/70 shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden ${isDragging ? "rotate-1 shadow-xl scale-105" : ""}`}
      >
        ...
        <div className="flex items-start gap-2 px-4 pt-4 pb-2">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <div className="h-7 w-7 rounded-lg bg-slate-900 flex items-center justify-center text-white font-black text-[11px] shrink-0">
                {quote.customerName.charAt(0).toUpperCase()}
              </div>
              <p className="font-black text-slate-900 text-sm truncate tracking-tight">{quote.customerName}</p>
            </div>
          </div>
          <span className="text-base font-black text-slate-900 shrink-0 tracking-tight">${quote.estimatedPrice.toFixed(0)}</span>
        </div>
        ...
        <div className="px-4 pb-3">
          <div className="flex items-center gap-1.5 text-xs font-bold text-slate-500 mt-1">
            ...
            <span className="ml-auto text-slate-400">{quote.distanceMiles.toFixed(1)} mi</span>
          </div>
        </div>
...
function OverlayCard({ quote }: { quote: Quote }) {
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-2xl rotate-2 scale-105 overflow-hidden w-64">
      <div className="flex items-start gap-2 px-4 pt-4 pb-2">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            <p className="font-black text-slate-900 text-sm truncate">{quote.customerName}</p>
          </div>
        </div>
        <span className="text-base font-black text-slate-900 shrink-0">${quote.estimatedPrice.toFixed(0)}</span>
      </div>
      <div className="px-4 pb-3">
        <div className="flex items-center gap-1.5 text-xs font-bold text-slate-500 mt-1">
          <MapPin size={10} className="text-red-400 shrink-0" />
        ...
        {COLUMNS.map((col) => {
          ...
          return (
            <motion.div
              ...
              className={`flex-shrink-0 w-64 rounded-2xl ${col.bg} border border-slate-200/60 flex flex-col`}
              id={col.id}
            >
              {/* Column Header */}
              <div className="px-4 py-3 flex items-center gap-2 border-b border-slate-200/60">
                ...
                <span className="ml-auto text-xs font-black text-slate-400 bg-white/70 px-2 py-0.5 rounded-full">
                  {colQuotes.length}
                </span>
              </div>
              ...
              <div className="flex-1 p-3 space-y-2.5 overflow-y-auto" id={col.id}>
                ...
                {colQuotes.length === 0 && (
                  <div className="flex items-center justify-center h-20 rounded-xl border-2 border-dashed border-slate-200 text-slate-300 text-xs font-bold">
                    Drop here
                  </div>
                )}
              </div>

...
export default function QuoteDocumentsCard({ quoteId }: { quoteId: string }) {
  ...
  const runAction = async (nextAction: "issue_quote" | "view_quote" | "email_quote") => {
    ...
    try {
      ...
      if (nextAction === "view_quote" && data.url) {
        ...
        <div className="space-y-3">
          ...
          {invoiceDocument && (
            <div className="rounded-xl border border-emerald-200/70 dark:border-emerald-500/20 bg-emerald-50/60 dark:bg-emerald-500/5 p-4">
              ...
              <div className="mt-3 rounded-lg bg-white/70 dark:bg-white/5 px-3 py-2 text-[11px] font-bold text-slate-500 dark:text-zinc-400">
                Invoice PDF coming next
              </div>
            </div>
          )}
        </div>
